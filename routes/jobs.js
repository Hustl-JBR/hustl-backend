...
// POST /jobs/:id/decline-price-change - DEPRECATED (Phase 2)
// Price changes after acceptance are no longer allowed for flat-rate jobs.
router.post('/:id/decline-price-change', authenticate, requireRole('CUSTOMER'), async (req, res) => {
  // Log deprecation attempt for monitoring
  console.warn(`[DEPRECATED] POST /jobs/${req.params.id}/decline-price-change attempted by user ${req.user.id}`);
